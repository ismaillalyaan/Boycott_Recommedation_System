...
app = Flask(__name__)
CORS(app, resources={r"/process_image": {"origins": "*"}, r"/add_product": {"origins": "*"}, r"/search_products": {"origins": "*"}})
...
# Microsoft Graph Config for Future Use
...
def index():
    return jsonify({
        "message": "Welcome to the RecSys API. Use POST /process_image to analyze an image, POST /add_product to add a product, or GET /search_products for autocomplete search."
    })
    ...
    try:
        ...
        if product:
            ...
            return jsonify({
                ...
                "is_boycotted": product['is_boycotted'],  # Add is_boycotted to response
