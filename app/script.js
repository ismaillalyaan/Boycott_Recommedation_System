document.addEventListener('DOMContentLoaded', function() {
    ...
    // Only define these functions if elements exist (for pages like index.html)
    ...
    window.processImage = async function() {
        ...
        try {
            ...
            console.log('Full response (processImage):', data); // Log full response for debugging
            ...
            // Use is_boycotted from response
            const boycotted = data.is_boycotted !== undefined ? Boolean(data.is_boycotted) : false; // Default to false if undefined
    ...
    async function fetchSuggestions(query) {
        try {
            const response = await fetch(`http://localhost:5000/search_products?query=${encodeURIComponent(query)}`);
            if (!response.ok) {
    ...
    async function searchProduct(productId) {
        ...
        try {
            const response = await fetch(`http://localhost:5000/search_products?query=${encodeURIComponent(searchInput.value)}`);
            if (!response.ok) {
                throw new Error('خطأ في الاتصال بالخادم');
            }
            ...
            try {
                const simResponse = await fetch(`http://localhost:5000/process_image`, {
                    method: 'POST',
                ...
                console.log('Full response (searchProduct):', simData); // Log full response for debugging
                ...
                // Use is_boycotted from response
                const boycotted = simData.is_boycotted !== undefined ? Boolean(simData.is_boycotted) : false; // Default to false if undefined
    ...
    // Animation for red arrow buttons
    ...
    if (arrowNavLinks.length > 0) {
        arrowNavLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                // Normalize current URL and target href for comparison
                const currentUrl = window.location.pathname.replace(/^\/|\/$/g, ''); // Remove leading/trailing slashes
                const targetPath = href.replace(/^\/|\/$/g, ''); // Remove leading/trailing slashes

                console.log('Navigating from:', currentUrl, 'to:', href); // Debug log

                // Always navigate if href is valid (removing the redundant condition)
                if (href) {
                    ...
                    // Navigate to the target page after animations start
                    setTimeout(() => {
                        window.location.href = href; // Use href directly
                    }, 1500);

                    // Continue animation (even though navigation has occurred)
                    setTimeout(() => {
